...
const selectAndPrintImages = () => {
  ...
  if (selection === 'unicorn') {
    selectedImageUrl = 'https://ih0.redbubble.net/image.412518259.2391/flat,550x550,075,f.u1.jpg';
  }

  if (selection === 'centuar') {
    selectedImageUrl = 'https://www.borisjulie.com/wp-content/uploads/2015/03/B-830.png';
  }

  if (selection === 'mermaid') {
    selectedImageUrl = mermaidUrl;
  }

  if (selection === 'dragon') {
    selectedImageUrl = dragonUrl;
  }

  if (selection === 'pegasus') {
    selectedImageUrl = pegasusUrl;
  }

  for (let i = 0; i < num; i++) {
    console.log(selectedImageUrl);
    document.write(selectedImageUrl);
  }
}